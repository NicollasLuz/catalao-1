...
public class SwerveSubsystem extends SubsystemBase {
  ...
  private boolean correctionPID = true;
    ...
    try {
      swerveDrive =
        new SwerveParser(directory)
          .createSwerveDrive(
            Tracao.MAX_SPEED,
            Dimensoes.ANGLE_CONVERSION,
            Dimensoes.DRIVE_CONVERSION
          );
    } catch (Exception e) {
  ...
  public void periodic() {
    ...
    SmartDashboard.putBoolean("Teste", correctionPID);
  }
  ...
  public ChassisSpeeds discretize(ChassisSpeeds speeds) {
    var desiredDeltaPose = new Pose2d(
      ...
      new Rotation2d(
        speeds.omegaRadiansPerSecond * Tracao.DT * Tracao.CONSTANT_ROTATION
      )
    );
